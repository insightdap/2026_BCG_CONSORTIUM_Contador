---
title: "Consortium Legal - Contador"
author: "[2026]"
output: 
  flexdashboard::flex_dashboard:
    logo: consortium_logo.png
    orientation: rows
...
principal <- read_sheet("17DRYDg1scxWggi3r2fS3n2sB2PVNWWRjC5RgJHaD9Lk")
...
principal <- principal %>% 
  left_join(select(data,ref,fecha,`Submission ID`),
            by="ref")
...
layout_column_wrap(
  ...
  card(
    ...
    card_body(
      ...
      count() %>% 
      ungroup() %>% 
      hchart(type="pie",hcaes(x=demo1,y=n),name="Género") %>% 
...
layout_column_wrap(
  ...
  card(
    ...
    card_body(
      ...
      count() %>% 
      ungroup() %>% 
      hchart(type="pie",hcaes(x=demo2,y=n),name="Nivel Jerárquico") %>% 
  ...
  card(
    ...
    card_body(
     principal %>%
      filter(estado=="Completado") %>% 
      group_by(floor_date(fecha, "day"),demo2) %>%
      count() %>% 
      rename("dias"=1) %>% 
      ungroup() %>% 
      mutate(tiempo = dias %>% datetime_to_timestamp()) %>% 
      hchart("spline", hcaes(x = tiempo, y = n, group = demo2)) %>%
      hc_title(text = "Respuestas por Nivel Jerárquico") %>%
      hc_subtitle(text = "Tendencia diaria") %>% 
      hc_xAxis(type = "datetime", labels = list(format = "{value:%d-%b}"), title = list(text = "")) %>% 
      hc_yAxis(title = list(text = "Cantidad de Respuestas")) %>%
...
layout_column_wrap(
  ...
  card(
    ...
    card_body(
      ...
      count() %>% 
      ungroup() %>% 
      hchart(type="pie",hcaes(x=demo8,y=n),name="Tiempo de Trabajo") %>% 
  ...
  card(
    ...
    card_body(
     principal %>%
      filter(estado=="Completado") %>% 
      group_by(floor_date(fecha, "day"),demo8) %>%
      count() %>% 
      rename("dias"=1) %>% 
      ungroup() %>% 
      mutate(tiempo = dias %>% datetime_to_timestamp()) %>% 
      hchart("spline", hcaes(x = tiempo, y = n, group = demo8)) %>%
      hc_title(text = "Respuestas por Tiempo de Trabajo") %>%
      hc_subtitle(text = "Tendencia diaria") %>% 
      hc_xAxis(type = "datetime", labels = list(format = "{value:%d-%b}"), title = list(text = "")) %>% 
      hc_yAxis(title = list(text = "Cantidad de Respuestas")) %>%
...
layout_column_wrap(
  ...
  card(
    ...
    card_body(
      ...
      count() %>% 
      ungroup() %>% 
      hchart(type="pie",hcaes(x=demo3,y=n),name="Generación") %>% 
  ...
  card(
    ...
    card_body(
     principal %>%
      filter(estado=="Completado") %>% 
      group_by(floor_date(fecha, "day"),demo3) %>%
      count() %>% 
      rename("dias"=1) %>% 
      ungroup() %>% 
      mutate(tiempo = dias %>% datetime_to_timestamp()) %>% 
      hchart("spline", hcaes(x = tiempo, y = n, group = demo3)) %>%
      hc_title(text = "Respuestas por Generación") %>%
      hc_subtitle(text = "Tendencia diaria") %>% 
      hc_xAxis(type = "datetime", labels = list(format = "{value:%d-%b}"), title = list(text = "")) %>% 
      hc_yAxis(title = list(text = "Cantidad de Respuestas")) %>%
...
layout_column_wrap(
  ...
  card_body(
   contador_pais %>% 
    ...
    hc_add_series(type = "bar",data = contador_pais,hcaes(x = pais, y = prop_Completado, z = respuestas_Completado),
